...
> 推荐您在使用前先通过互联网了解一下ES原生的父子类型或ElasticsearchClient中父子类型相关的索引创建及CRUD,有一定理论基础后可以更方便理解本框架的API设计及使用. ES的父子类型及嵌套类型本身就是极度复杂的东西,使用成本非常高,在看完原生语法后相信您会给我们Star!

## 父子类型创建索引

- 步骤一 添加注解,指定父子关系,父子节点继承框架内置的BaseJoin类(必须):
```java
/**
