...
为什么使用LambdaEsUpdateWrapper更新数据时，报空指针异常
首先因为更新操作，本身是先基于条件查询数据，也就是这个方法：this.selectListByUpdateWrapper(updateWrapper, indexName);，再基于id维度进行更新操作，由于这个查询返回的是一个list集合，会对查询结果进行类型转换：return (List)Arrays.stream(searchHits).map(this::parseOne).collect(Collectors.toList());，这时如果我们的实体类没有无参构造方法时，就会出现转换错误，生成一个空对象的list，当执行循环跟新时：list.forEach((item)，item为null，就出现了空指针。

为什么使用LambdaEsUpdateWrapper更新数据时，符合匹配调的数据没有全部更新
首先更新是先基于条件查询数据，再基于主键Id维度更新，this.selectListByUpdateWrapper(updateWrapper, indexName);方法中，有一个获取查询结果阈值的逻辑：int size = (Integer)Optional.ofNullable(entityInfo).map(EntityInfo::getMaxResultWindow).orElse(GlobalConfigCache.getGlobalConfig().getDbConfig().getBatchUpdateThreshold());
可以看到首先是基于实体类配置的MaxResultWindow进行设置阈值，如果为空会去DbConfig.batchUpdateThreshold，可以看到DbConfig的初始方法中，默认值是10000。所以更新操作存在更新数据不全的风险，可以增加一个循环策略，直到返回值为0.
